"""Export a no-write V1.5 CO2 SENCO5 final linear-trim review."""
...
from ..validation.co2_senco5_linear_trim_review import (
    Co2Senco5LinearTrimConfig,
    write_co2_senco5_linear_trim_review,
)
...
def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="No-write V1.5 CO2 SENCO5 linear-trim review.")
    ...
    parser.add_argument("--max-abs-c0-ppm", type=float, default=100.0)
    parser.add_argument("--max-abs-c1-delta", type=float, default=0.10)
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        cfg = Co2Senco5LinearTrimConfig(
            ...
            max_abs_c0_ppm=float(args.max_abs_c0_ppm),
            max_abs_c1_delta=float(args.max_abs_c1_delta),
        )
        outputs = write_co2_senco5_linear_trim_review(
            verification_summary_csv=args.verification_summary_csv,
            output_dir=args.output_dir,
            cfg=cfg,
        )
    except Exception as exc:
        print(f"V1.5 CO2 SENCO5 linear-trim review failed: {exc}", file=sys.stderr, flush=True)
        return 1
