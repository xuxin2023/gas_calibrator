"""Run controlled V1.5 H2O SENCO6 neutral linear-trim clears.

SENCO6 is treated as the final H2O concentration affine trim:
corrected_h2o = h2o * C1 + C0.

Bench evidence on 2026-05-29 showed analyzer 033 had GETCO6=[24.4,1.0],
which adds about 24.4 mmol/mol to H2O on dry CO2 gas. The analyzer firmware then
uses H2O in its displayed CO2 dry-basis correction, so this non-neutral H2O trim
inflates the final CO2 ppm even when the raw SENCO1/SENCO3 CO2 chain is correct.

Neutralization uses the dedicated CLEARSENCO6 command and verifies by GETCO6
readback. This tool never writes SENCO1/SENCO2/SENCO3/SENCO4/SENCO5, never
changes analyzer IDs, and never controls PACE, valves, water routes, gas routes,
or humidity generation.
"""
...
CONFIRMATION_TEXT = "WRITE_SENCO6_NEUTRAL_V1_5_H2O_LINEAR_TRIM"
TARGET_SENCO6 = (0.0, 1.0)


def _format_decimal(value: Any, *, decimals: int = 1) -> str:
    numeric = float(value)
    if not math.isfinite(numeric):
        raise ValueError("SENCO6 linear trim coefficient must be finite")
...
def _clear_senco6(
    ...
    *,
    ...
    payload = f"CLEARSENCO6,YGAS,{target}"
...
def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Controlled V1.5 H2O SENCO6 neutral linear-trim writer.")
    ...
    parser.add_argument("--write-all-nonneutral", action="store_true", help="Write every selected non-neutral SENCO6.")
    ...
    parser.add_argument("--decimal-places", type=int, default=1, help="Deprecated; neutralization uses CLEARSENCO6.")
    parser.add_argument(
        "--senco6-target-mode",
        choices=("broadcast", "device_id"),
        default="broadcast",
        help="Use CLEARSENCO6,YGAS,FFF or CLEARSENCO6,YGAS,<device_id> for neutralization.",
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        require_fragile_serial_timing(args, tool_name="run_v1_5_h2o_senco6_neutral_controlled_write")
    except ValueError as exc:
        print(str(exc), file=sys.stderr)
        return 2

    if not bool(args.enable_senco6_write) or str(args.operator_confirmation).strip() != CONFIRMATION_TEXT:
        print("SENCO6 write locked: explicit unlock and operator confirmation are required.", file=sys.stderr)
        return 2
    if not str(args.reviewer).strip() or not str(args.approver).strip():
        print("SENCO6 write locked: reviewer and approver are required.", file=sys.stderr)
        return 2
    ...
    for idx, device_id in enumerate(target_ids):
        ...
        try:
            base._log(f"H2O SENCO6 neutral controlled write begin: device_id={device_id} port={analyzer_cfg.get('port')}")
            ga.open()
            ...
            if _is_neutral(old_live, atol=float(args.compare_atol)):
                ...
                if not args.write_all_nonneutral and not selected:
                    ...
                    acked, payload, write_ack_lines = _clear_senco6(
                        ga,
                        device_id=device_id,
                    ...
                    write_applied = True
                    if not acked:
                        raise RuntimeError(f"SENCO6_WRITE_ACK_FAILED response={write_ack_lines}")
                    ...
                    if not _is_neutral(final_live, atol=float(args.compare_atol)):
                        raise RuntimeError(f"SENCO6_READBACK_MISMATCH final={final_live}")
        ...
        rows.append(
            {
                ...
                "target_senco6": json.dumps(list(TARGET_SENCO6), ensure_ascii=False),
                "final_senco6": json.dumps(final_live, ensure_ascii=False),
                "payload": payload,
                "payload_target_mode": str(args.senco6_target_mode),
                ...
                "writes_senco4": False,
                "writes_senco5": False,
                "writes_senco6": bool(write_applied),
                "writes_device_id": False,
                "clears_senco": bool(write_applied),
                "clears_senco6": bool(write_applied),
    ...
    base._write_csv(output_dir / "senco6_neutral_write_events.csv", rows)
    ...
    (output_dir / "senco6_neutral_write_meta.json").write_text(
        json.dumps(
            {
                "created_at": datetime.now().isoformat(timespec="seconds"),
                "tool": "run_v1_5_h2o_senco6_neutral_controlled_write",
                "confirmation_text": CONFIRMATION_TEXT,
                "reviewer": args.reviewer,
                "approver": args.approver,
                "target_senco6": list(TARGET_SENCO6),
                "decimal_places": int(args.decimal_places),
                "senco6_target_mode": str(args.senco6_target_mode),
                "senco6_ack_timeout_s": float(args.senco6_ack_timeout_s),
                "writes_senco1": False,
                "writes_senco2": False,
                "writes_senco3": False,
                "writes_senco4": False,
    ...
    metadata = ValidationMetadata(
        tool_name="run_v1_5_h2o_senco6_neutral_controlled_write",
        analyzers=[str(row.get("device_id") or "") for row in rows],
        config_path=str(Path(args.config).resolve()),
        output_dir=str(output_dir),
        config_summary={
            "suite_id": "v1_5_h2o_senco6_linear_trim_controlled_write",
        ...
        notes=[
            "Controlled real-device H2O SENCO6 neutral linear-trim clear.",
            "SENCO6 applies corrected H2O = H2O*C1 + C0; neutral target is C0=0.0,C1=1.0.",
            "Neutralization uses CLEARSENCO6 and must be verified by GETCO6 readback.",
            "No PACE, valve, water route, gas route, humidity generator, device-ID writes, or SENCO1/SENCO2/SENCO3/SENCO4/SENCO5 writes are performed.",
        ],
    )
    write_validation_report(
        output_dir,
        tables={
            "senco6_neutral_write_events": rows,
            "conclusion": [conclusion],
        },
        metadata=metadata,
        prefix="h2o_senco6_neutral_controlled_write",
    )
