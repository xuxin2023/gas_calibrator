"""Export the offline V1.5 historical replay contract guard."""
...
from ..validation.v1_5_historical_replay_contract import (
...
def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the offline V1.5 historical replay contract: legacy replay stays ratio/45/13, "
            "new algorithm replay stays absorption/R0 shadow evidence, and replay cannot authorize release."
        )
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        outputs = write_v1_5_historical_replay_contract(
            profile_path=args.profile_path,
            output_dir=args.output_dir,
        )
        model = build_v1_5_historical_replay_contract(profile_path=args.profile_path)
    except Exception as exc:
        print(f"V1.5 historical replay contract export failed: {exc}", file=sys.stderr, flush=True)
        return 1
