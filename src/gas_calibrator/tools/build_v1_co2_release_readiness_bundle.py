"""Build a V1 CO2 release readiness evidence merge bundle."""
...
from ..workflow.co2_release_readiness_bundle import (
    build_co2_release_readiness_bundle,
    write_co2_release_readiness_artifacts,
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a V1 CO2 release readiness evidence merge bundle from existing fit and sampling sidecars."
    )
    parser.add_argument("--fit-arbitration-summary-json", type=Path, help="Existing fit_arbitration_summary.json path.")
    parser.add_argument("--sampling-settle-summary-json", type=Path, help="Existing sampling_settle_summary.json path.")
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory for release readiness artifacts.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_mode = args.fit_arbitration_summary_json is not None or args.sampling_settle_summary_json is not None
    raw_inputs = [value for value in (args.run_dir, args.points_csv, args.candidate_pack_json) if value is not None]
    if json_mode:
        if args.fit_arbitration_summary_json is None or args.sampling_settle_summary_json is None:
            parser.error("must provide both --fit-arbitration-summary-json and --sampling-settle-summary-json")
        if raw_inputs:
        ...
        payload = build_co2_release_readiness_bundle(
            ...
            sampling_settle_payload=_load_json_payload(args.sampling_settle_summary_json),
        )
    else:
        if len(raw_inputs) != 1:
        ...
        payload = build_co2_release_readiness_bundle(rows)

    write_co2_release_readiness_artifacts(args.output_dir, payload)
    return 0
