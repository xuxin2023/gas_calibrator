"""Export the V1.5 read-only COM minimal executor implementation review."""
...
from ..validation.v1_5_formal_readonly_com_minimal_executor_review import (
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write an offline implementation-review sidecar for the future minimal V1.5 "
            "read-only COM executor. This command never opens analyzer COM ports."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Return exit code 3 when the review sidecar requires review.",
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if _locked_live_option_requested(args):
        print(
            "V1.5 read-only COM minimal executor review is offline only. "
    ...
    try:
        model = build_v1_5_formal_readonly_com_minimal_executor_review(
            formal_readonly_com_execution_plan_preview_json=args.formal_readonly_com_execution_plan_preview_json,
        )
        ...
        print(
            f"V1.5 read-only COM minimal executor review failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return 1
