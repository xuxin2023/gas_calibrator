"""Build a V1 CO2 engineer review worklist / decision packet bundle."""
...
from ..workflow.co2_engineer_review_worklist_bundle import (
    build_co2_engineer_review_worklist_bundle,
    write_co2_engineer_review_worklist_artifacts,
)
from ..workflow.co2_fit_arbitration_bundle import build_co2_fit_arbitration_bundle
from ..workflow.co2_fit_evidence_coverage_bundle import build_co2_fit_evidence_coverage_bundle
from ..workflow.co2_point_evidence_provenance_bundle import build_co2_point_evidence_provenance_bundle
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a V1 CO2 engineer review worklist / decision packet bundle."
    )
    parser.add_argument("--fit-arbitration-summary-json", type=Path, help="Existing fit_arbitration_summary.json path.")
    parser.add_argument("--fit-evidence-coverage-summary-json", type=Path, help="Existing fit_evidence_coverage_summary.json path.")
    parser.add_argument("--release-readiness-summary-json", type=Path, help="Existing release_readiness_summary.json path.")
    parser.add_argument("--point-evidence-provenance-summary-json", type=Path, help="Existing point_evidence_provenance_summary.json path.")
    ...
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory for engineer review bundle artifacts.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_inputs = [
        args.fit_arbitration_summary_json,
        args.fit_evidence_coverage_summary_json,
        args.release_readiness_summary_json,
        args.point_evidence_provenance_summary_json,
    ]
    json_mode = any(value is not None for value in json_inputs)
    raw_inputs = [value for value in (args.run_dir, args.points_csv, args.candidate_pack_json) if value is not None]

    if json_mode:
        if any(value is None for value in json_inputs):
            parser.error(
                "must provide --fit-arbitration-summary-json, --fit-evidence-coverage-summary-json, "
                "--release-readiness-summary-json, and --point-evidence-provenance-summary-json together"
            )
        if raw_inputs:
            parser.error(
                "cannot combine summary-json inputs with --run-dir / --points-csv / --candidate-pack-json"
            )
        payload = build_co2_engineer_review_worklist_bundle(
            ...
            fit_evidence_coverage_payload=_load_json_payload(args.fit_evidence_coverage_summary_json),
            release_readiness_payload=_load_json_payload(args.release_readiness_summary_json),
            provenance_payload=_load_json_payload(args.point_evidence_provenance_summary_json),
        )
    else:
        if len(raw_inputs) != 1:
        ...
        coverage_payload = build_co2_fit_evidence_coverage_bundle(
            rows,
    ...
    write_co2_engineer_review_worklist_artifacts(args.output_dir, payload)
    return 0
