"""Record that reviewed V1.5 next-step execution remains unavailable."""
...
from ..validation.v1_5_authoritative_resume_offline_state_advance_next_step_blocked_executor import (
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument("--next-step-authorization-preflight-json", required=True)
    ...
    model = build_v1_5_authoritative_resume_offline_state_advance_next_step_blocked_executor(
        next_step_authorization_preflight_json=(
            args.next_step_authorization_preflight_json
        )
    )
    outputs = write_v1_5_authoritative_resume_offline_state_advance_next_step_blocked_executor(
