"""Run the controlled V1.5 read-only COM preflight executor in blocked mode only."""
...
from ..validation.v1_5_formal_initialization_readonly_com_preflight_controlled_blocked_executor import (
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write a no-COM/no-write blocked executor artifact for the future controlled V1.5 "
            "read-only initialization COM preflight executor. This command never opens analyzer serial ports."
        )
    )
    parser.add_argument("--formal-initialization-readonly-com-preflight-controlled-executor-design-json", required=True)
    ...
    forbidden = parser.add_argument_group("locked controlled read-only real-COM options")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if _locked_live_option_requested(args):
        print(
            "V1.5 controlled read-only initialization COM preflight execution is locked in this command. "
            "Real-COM preflight must be implemented in a future separately reviewed executor.",
            file=sys.stderr,
            flush=True,
        )
        return 2
    try:
        model = build_v1_5_formal_initialization_readonly_com_preflight_controlled_blocked_executor(
            formal_initialization_readonly_com_preflight_controlled_executor_design_json=(
                args.formal_initialization_readonly_com_preflight_controlled_executor_design_json
            ),
        )
        ...
        print(
            f"V1.5 controlled read-only initialization COM preflight blocked executor failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return 1
