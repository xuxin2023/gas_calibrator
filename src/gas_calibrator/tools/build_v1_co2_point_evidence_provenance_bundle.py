"""Build a V1 CO2 point evidence provenance / source-segment lineage bundle."""
...
from ..workflow.co2_point_evidence_provenance_bundle import (
    build_co2_point_evidence_provenance_bundle,
    write_co2_point_evidence_provenance_artifacts,
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a V1 CO2 point evidence provenance / source-segment lineage bundle."
    )
    parser.add_argument(
        "--fit-evidence-coverage-summary-json",
        type=Path,
        help="Existing fit_evidence_coverage_summary.json path.",
    )
    parser.add_argument(
        "--release-readiness-summary-json",
        type=Path,
        help="Existing release_readiness_summary.json path.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory for point evidence provenance artifacts.",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    json_mode = (
        args.fit_evidence_coverage_summary_json is not None
        or args.release_readiness_summary_json is not None
    )
    raw_inputs = [value for value in (args.run_dir, args.points_csv, args.candidate_pack_json) if value is not None]

    if json_mode:
        if args.fit_evidence_coverage_summary_json is None or args.release_readiness_summary_json is None:
            parser.error(
                "must provide both --fit-evidence-coverage-summary-json and --release-readiness-summary-json"
            )
        if raw_inputs:
            parser.error(
        ...
        release_payload = build_co2_release_readiness_bundle(rows)
    ...
    write_co2_point_evidence_provenance_artifacts(args.output_dir, payload)
    return 0
