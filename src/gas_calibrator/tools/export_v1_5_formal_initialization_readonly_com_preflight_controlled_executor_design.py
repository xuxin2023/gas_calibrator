"""Export the offline V1.5 read-only COM preflight controlled executor design."""
...
from ..validation.v1_5_formal_initialization_readonly_com_preflight_controlled_executor_design import (
...
def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export offline/no-COM V1.5 controlled read-only initialization COM preflight "
            "executor design artifacts. This does not open COM ports, write SN/device IDs, "
            "write SENCO, or connect PostgreSQL."
        )
    )
    parser.add_argument(
        "--formal-initialization-readonly-com-preflight-blocked-executor-json",
        default=None,
        help="Blocked read-only COM preflight executor JSON that this controlled design builds upon.",
    )
    ...
    try:
        outputs = write_v1_5_formal_initialization_readonly_com_preflight_controlled_executor_design(
            ...
            formal_initialization_readonly_com_preflight_blocked_executor_json=(
                args.formal_initialization_readonly_com_preflight_blocked_executor_json
            ),
        )
        ...
        print(
            f"V1.5 read-only COM preflight controlled executor design export failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return 1
