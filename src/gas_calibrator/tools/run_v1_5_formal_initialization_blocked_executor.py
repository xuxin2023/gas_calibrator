"""Run the V1.5 formal initialization executor only in blocked-stub mode."""
...
from ..validation.v1_5_formal_initialization_blocked_executor import (
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write a no-COM/no-write blocked executor artifact for future V1.5 formal initialization automation. "
            "This command never executes the initialization plan."
        )
    )
    parser.add_argument("--formal-initialization-executor-dry-run-json", required=True)
    ...
    forbidden = parser.add_argument_group("locked live initialization options")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if _locked_live_option_requested(args):
        print(
            "V1.5 live initialization execution is locked in this command. "
            "Real-COM and controlled-write initialization must be implemented in a future separately reviewed executor.",
            file=sys.stderr,
            flush=True,
        )
        return 2
    try:
        model = build_v1_5_formal_initialization_blocked_executor(
            formal_initialization_executor_dry_run_json=args.formal_initialization_executor_dry_run_json,
            formal_initialization_plan_json=args.formal_initialization_plan_json,
        )
        outputs = write_v1_5_formal_initialization_blocked_executor_outputs(model, args.output_dir)
    except Exception as exc:
        print(f"V1.5 formal initialization blocked executor failed: {exc}", file=sys.stderr, flush=True)
        return 1
    result = {
