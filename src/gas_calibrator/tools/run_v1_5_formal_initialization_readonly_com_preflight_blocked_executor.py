"""Run the V1.5 read-only initialization COM preflight only in blocked-stub mode."""
...
from ..validation.v1_5_formal_initialization_readonly_com_preflight_blocked_executor import (
...
def _parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write a no-COM/no-write blocked executor artifact for future V1.5 read-only "
            "initialization COM preflight. This command never opens analyzer serial ports."
        )
    )
    parser.add_argument("--formal-initialization-readonly-com-preflight-design-json", required=True)
    ...
    forbidden = parser.add_argument_group("locked read-only real-COM options")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if _locked_live_option_requested(args):
        print(
            "V1.5 read-only initialization COM preflight execution is locked in this command. "
    ...
    try:
        model = build_v1_5_formal_initialization_readonly_com_preflight_blocked_executor(
            formal_initialization_readonly_com_preflight_design_json=(
                args.formal_initialization_readonly_com_preflight_design_json
            ),
        )
        ...
        print(
            f"V1.5 read-only initialization COM preflight blocked executor failed: {exc}",
            file=sys.stderr,
            flush=True,
        )
        return 1
