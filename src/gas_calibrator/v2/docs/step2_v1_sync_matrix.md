# Step 2 V2-Only Sync Audit Matrix（历史文档，已废止）

> 2026-07-28：V1.5 已成为唯一最终产品。AppFacade、PlanGateway、可编辑计划页和旧 V2 产品 shell 已删除；下表只保留历史审计语境，不得用来恢复 V2 UI 或平行校准主链。
...
| CO2 post-seal quality guards | `src/gas_calibrator/workflow/runner.py` | already synced | already synced | `src/gas_calibrator/v2/core/services/qc_service.py`, `src/gas_calibrator/v2/core/result_store.py`, `src/gas_calibrator/v2/core/offline_artifacts.py` | `tests/test_runner_co2_quality_guards.py`, `tests/v2/test_qc_service.py`, `tests/v2/test_offline_artifacts.py` | Rebound veto, pressure-scaled dewpoint QC, and stale gauge metrics are already present in V2 QC/artifacts/review surfaces; the retired measurement analytics service is not part of this evidence chain. |
| pressure point selection ambient semantics | `src/gas_calibrator/workflow/runner.py`, `tests/test_pressure_point_selection.py` | already synced | historical compiler contract only | `src/gas_calibrator/v2/domain/pressure_selection.py`, `src/gas_calibrator/v2/config/models.py`, `src/gas_calibrator/v2/core/plan_compiler.py` | `tests/test_pressure_point_selection.py`, `tests/v2/test_config_models.py`, `tests/v2/test_plan_compiler.py` | V2 profile editing has retired. V1.5 only previews the mature fixed queue and must not gain a second editable point source. |
| room-temp pressure diagnostic / plots / analyzer-chain isolation | `src/gas_calibrator/validation/room_temp_co2_pressure_diagnostic.py`, `src/gas_calibrator/validation/room_temp_co2_pressure_plots.py` | partially synced | offline-only | `src/gas_calibrator/v2/core/offline_artifacts.py`, `src/gas_calibrator/v2/scripts/historical_artifacts.py` | `tests/test_room_temp_co2_pressure_diagnostic.py`, `tests/v2/test_build_offline_governance_artifacts.py`, `tests/v2/test_historical_artifacts_cli.py` | High-value engineering analysis remains offline/read-only; deleted V2 UI surfaces must not be restored. |
...
| logging/export taxonomy parity | `src/gas_calibrator/logging_utils.py` | partially synced | read-only compatibility | `src/gas_calibrator/v2/core/result_store.py`, `src/gas_calibrator/v2/core/offline_artifacts.py`, `src/gas_calibrator/v2/scripts/historical_artifacts.py` | `tests/v2/test_result_store.py`, `tests/v2/test_historical_artifacts_cli.py` | Preserve field taxonomy for historical artifacts only; deleted UI/workbench consumers must not be restored. |
