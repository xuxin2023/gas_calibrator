...
from .pressure_control_service import (
...
class AIExplanationService:
    ...
    def generate_ai_anomaly_report(self, advisor: Any) -> str:
        ...
        if not failed_points and not alarms:
            return ""
        diagnosis = advisor.diagnose_run(failed_points=failed_points, device_events=device_events, alarms=alarms)
        if not diagnosis:
            return ""
    ...
    def generate_ai_run_summary(self, summarizer: Any, *, anomaly_diagnosis: str = "") -> str:
        if summarizer is None:
            return ""
        text = summarizer.write_summary(self.context.session.output_dir, anomaly_diagnosis=anomaly_diagnosis)
        if text:
    ...
    def generate_ai_outputs(self) -> None:
        ...
        try:
            self.generate_ai_run_summary(runtime.summarizer, anomaly_diagnosis=anomaly_text)
        except Exception as exc:
