...
class ConditioningService:
    ...
    def _verify_co2_preseal_atmosphere_hold_pressure(self, point: CalibrationPoint) -> str:
        ...
        if hard_abort_pressure_hpa is not None and float(pressure_hpa) >= float(
            hard_abort_pressure_hpa
        ):
            self.host._record_workflow_timing(
                "co2_preseal_atmosphere_hold_pressure_guard",
                "fail",
        ...
        if urgent_seal_threshold_hpa is not None and float(pressure_hpa) >= float(
            urgent_seal_threshold_hpa
        ):
            decision = "positive_preseal_arm_handoff"
        elif ready_pressure_hpa is not None and float(pressure_hpa) >= float(
            ready_pressure_hpa
        ):
