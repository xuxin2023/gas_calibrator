...
_PRESSURE_TRACE_FIELDS = [
    ...
    "vent_status_after_vent0_classification",
    "outp_status_after_vent0",
    "outp_mode_after_vent0",
    "isol_status_after_vent0",
    "syst_err_after_vent0",
    "pace_pressure_after_vent0_hpa",
    "com22_pressure_after_vent0_hpa",
    ...
    "co2_pre_route_idle_isolation_skipped",
    "skipped_reason",
    "pace_vent0_before_route_open_suppressed",
    "route_baseline_output_state_before",
    "route_baseline_outp0_sent",
    "route_baseline_outp0_skipped_reason",
    "route_baseline_isol_state_before",
    "route_baseline_isol1_sent",
    "route_baseline_isol1_skipped_reason",
    "route_baseline_vent1_sent",
    ...
    "sealed_vent1_count",
    "sealed_vent0_count",
    "sealed_abnormal_vent_status_count",
    "sealed_pressure_control_vent_on_count",
    "sealed_outp1_count",
    "sealed_outp0_count",
    "sealed_setpoint_count",
    "sealed_sample_count",
    "sealed_actual_open_valves",
    "no_vent_guard_blocked_count",
...
class CalibrationRunner:
    ...
    def __init__(self, config: Dict[str, Any], devices: Dict[str, Any], logger: RunLogger, log_fn, status_fn):
        ...
        self._last_vent0_command_ts: Optional[float] = None
    ...
    def _engineering_safe_stop_pressure_relief_check_enabled(self) -> bool:
        return bool(self._wf("workflow.pressure.engineering_safe_stop_pressure_relief_check", False))

    def _safe_stop_pressure_relief_threshold_hpa(self) -> float:
        return max(0.0, float(self._wf("workflow.pressure.safe_stop_pressure_relief_threshold_hpa", 20.0) or 20.0))

    def _safe_stop_pressure_relief_reference_hpa(self) -> float:
        cfg_value = self._as_float(self._wf("workflow.pressure.safe_stop_pressure_relief_reference_hpa", None))
        if cfg_value is not None:
            return float(cfg_value)
        if self._atmosphere_reference_hpa is not None:
            return float(self._atmosphere_reference_hpa)
        return 1013.25

    def _engineering_safe_stop_pressure_relief_issue(self, result: Mapping[str, Any]) -> Optional[str]:
        if not self._engineering_safe_stop_pressure_relief_check_enabled():
            return None
        reference_hpa = self._safe_stop_pressure_relief_reference_hpa()
        threshold_hpa = self._safe_stop_pressure_relief_threshold_hpa()
        limit_hpa = reference_hpa + threshold_hpa
        readings = {
            "pace_pressure_hpa": self._as_float(result.get("pace_pressure_hpa")),
            "gauge_pressure_hpa": self._as_float(result.get("gauge_pressure_hpa")),
        }
        high = {
            key: value
            for key, value in readings.items()
            if value is not None and float(value) > float(limit_hpa)
        }
        if not high:
            return None
        details = ",".join(f"{key}={float(value):.3f}" for key, value in sorted(high.items()))
        return (
            "SAFE_STOP_PRESSURE_REMAINS_HIGH"
            f": reference_hpa={reference_hpa:.3f} threshold_hpa={threshold_hpa:.3f} "
            f"limit_hpa={limit_hpa:.3f} {details}"
        )
    ...
    def _restore_baseline_after_run(self) -> None:
        ...
        pace_pressure = self._as_float(result.get("pace_pressure_hpa"))
        gauge_pressure = self._as_float(result.get("gauge_pressure_hpa"))
        summary["safe_stop_pace_pressure_hpa"] = pace_pressure
        summary["safe_stop_com22_pressure_hpa"] = gauge_pressure
        issues = list(result.get("safe_stop_issues") or [])
        pressure_relief_issue = self._engineering_safe_stop_pressure_relief_issue(result)
        residual_pressure_cleared = True
        residual_pressure_high_reason = ""
        residual_pressure_requires_manual_clearance = False
        if pressure_relief_issue:
            issues.append(pressure_relief_issue)
            summary["safe_stop_pressure_relief_issue"] = pressure_relief_issue
            residual_pressure_cleared = False
            residual_pressure_high_reason = pressure_relief_issue
            residual_pressure_requires_manual_clearance = True
        summary["residual_pressure_cleared"] = residual_pressure_cleared
        summary["residual_pressure_high_reason"] = residual_pressure_high_reason
        summary["residual_pressure_requires_manual_clearance"] = residual_pressure_requires_manual_clearance
        if issues:
    ...
    @staticmethod
    def _co2_route_conditioning_baseline_reason(reason: str = "") -> bool:
        return str(reason or "").strip().lower() == "before co2 route conditioning"

    def _route_baseline_send_on_unknown_state(self) -> bool:
        return bool(self._wf("workflow.pressure.route_baseline_send_on_unknown_state", True))

    def _route_open_clean_atmosphere_vent1_max_age_s(self) -> float:
        return max(
            0.1,
            float(self._wf("workflow.pressure.route_open_clean_atmosphere_vent1_max_age_s", 3.0) or 3.0),
        )

    def _route_open_clean_atmosphere_pressure_tolerance_hpa(self) -> float:
        fallback = self._wf("workflow.pressure.safe_stop_pressure_relief_threshold_hpa", 20.0)
        return max(
            0.1,
    ...
    def _read_pressure_controller_int_state(self, pace: Any, getter_name: str) -> Optional[int]:
        ...
        if not callable(getter):
            return None
        try:
            return self._as_int(getter())
        except Exception:
            return None

    def _pressure_controller_atmosphere_hold_active(self, pace: Any = None) -> bool:
        pace = pace if pace is not None else self.devices.get("pace")
        getter = getattr(pace, "is_atmosphere_hold_active", None) if pace is not None else None
        if callable(getter):
            try:
                if bool(getter()):
                    return True
                ...
                pass
        return bool(self._pressure_atmosphere_hold_enabled)
    ...
    def _controlled_verified_exit_atmosphere_enabled(self, route_name: str) -> bool:
        return (
            self._controlled_outp_seal_transition_enabled(route_name)
            and self._open_flow_output_off_mode()
            and bool(self._wf("workflow.pressure.controlled_verified_exit_atmosphere", False))
        )
    ...
    def _pace_vent_status_classification(self, vent_status: Any, *, stage: str = "") -> str:
        value = self._as_int(vent_status)
        stage_text = str(stage or "").strip().lower()
        if value is None:
            return "unavailable_not_ready"
        if stage_text == "open_flow" and value == 1:
            return "open_flow_venting_ok"
        if value == 0:
            return "vent_ok_ready"
        if value == 1:
            return "vent_in_progress_not_ready"
        if value == 2:
    ...
    def _sealed_sweep_counter_names(self) -> Tuple[str, ...]:
        return (
            ...
            "atmosphere_transition_count",
            "vent_on_count",
            "vent_off_count",
        )

    def _sealed_sweep_context_for_counters(self) -> Optional[Dict[str, Any]]:
        if isinstance(self._co2_sealed_no_vent_guard_context, dict):
            return self._co2_sealed_no_vent_guard_context
        if isinstance(self._active_co2_sealed_sweep_context, dict):
            return self._active_co2_sealed_sweep_context
        return None

    def _increment_sealed_sweep_counter(self, key: str, amount: int = 1) -> None:
        for context in (self._co2_sealed_no_vent_guard_context, self._active_co2_sealed_sweep_context):
            if not isinstance(context, dict):
                continue
            context[key] = int(context.get(key) or 0) + int(amount)

    def _sealed_sweep_trace_extra(self, extra: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
        context = self._sealed_sweep_context_for_counters()
        values: Dict[str, Any] = {}
        if isinstance(context, dict):
            for key in self._sealed_sweep_counter_names():
                if key in _PRESSURE_TRACE_FIELDS:
                    values[key] = context.get(key, 0)
            values["sealed_actual_open_valves"] = context.get("sealed_actual_open_valves", "")
        if extra:
            values.update(dict(extra))
        return values

    def _controlled_exit_wait_for_vent_idle_timeout_s(self) -> float:
        return max(
            0.0,
            float(self._wf("workflow.pressure.controlled_exit_wait_for_vent_idle_timeout_s", 3.0) or 0.0),
        )

    def _controlled_exit_wait_for_vent_idle_poll_s(self) -> float:
        return max(
            0.05,
    ...
    def _build_co2_sealed_sweep_key(self, point: CalibrationPoint) -> Tuple[Any, ...]:
        ...
        ppm = self._as_float(point.co2_ppm)
        group = _normalized_co2_group(getattr(point, "co2_group", "")) or "A"
        source_valve = self._source_valve_for_point(point)
        try:
            route_key = tuple(self._co2_open_valves(point, include_total_valve=True))
        except Exception:
            route_key = ()
        return (
            "co2",
            None if temp is None else round(float(temp), 6),
    ...
    def _activate_co2_sealed_no_vent_guard(self, point: CalibrationPoint, *, reason: str = "") -> None:
        self._co2_sealed_no_vent_guard_active = True
        started_ts = time.time()
        self._co2_sealed_no_vent_guard_context = {
            "sealed_sweep_active": True,
            "sealed_sweep_route": "co2",
            "sealed_sweep_started_at": started_ts,
            "phase": "co2",
            "point_row": getattr(point, "index", None),
            "target_pressure_hpa": self._as_float(point.target_pressure_hpa),
            "key": self._build_co2_sealed_sweep_key(point),
            "activated_wall_ts": started_ts,
            "reason": reason,
            "output_enabled_once": False,
            "route_close_verified": True,
            "sealed_actual_open_valves": self._cached_actual_open_valves(),
        }
        for key in self._sealed_sweep_counter_names():
            self._co2_sealed_no_vent_guard_context.setdefault(key, 0)
        self._append_pressure_trace_row(
            point=point,
            route="co2",
            point_phase="co2",
            trace_stage="sealed_no_vent_guard_active",
            pressure_target_hpa=point.target_pressure_hpa,
            refresh_pace_state=False,
            extra_fields=self._sealed_sweep_trace_extra(
                {"sealed_actual_open_valves": self._cached_actual_open_valves()}
            ),
            note=reason or "CO2 route sealed; no-vent guard active",
        )

    def _clear_active_co2_sealed_sweep_context(self, *, reason: str = "", point: Optional[CalibrationPoint] = None) -> None:
        had_context = self._active_co2_sealed_sweep_context is not None
        had_guard = bool(self._co2_sealed_no_vent_guard_active)
        if had_context or had_guard:
            self._append_pressure_trace_row(
                point=point,
                route="co2",
                point_phase="co2",
                ...
                refresh_pace_state=False,
                extra_fields=self._sealed_sweep_trace_extra(),
                note=f"sealed_sweep_context_clear_reason={reason or 'unspecified'}",
            )
    ...
    def _begin_active_co2_sealed_sweep_context(self, point: CalibrationPoint) -> None:
        ...
        self._active_co2_sealed_sweep_context = {
            ...
            "phase": "co2",
            "key": key,
            "temp_c": self._as_float(point.temp_chamber_c),
            "co2_ppm": self._as_float(point.co2_ppm),
            "co2_group": _normalized_co2_group(getattr(point, "co2_group", "")) or "A",
            "source_valve": self._source_valve_for_point(point),
            "route_key": key[-1],
            "sealed_at_point_row": getattr(point, "index", None),
            "first_pressure_hpa": self._as_float(point.target_pressure_hpa),
        ...
        if isinstance(self._co2_sealed_no_vent_guard_context, dict):
            for counter_key in self._sealed_sweep_counter_names():
                self._active_co2_sealed_sweep_context[counter_key] = int(
                    self._co2_sealed_no_vent_guard_context.get(counter_key) or 0
                )
        self._append_pressure_trace_row(
            point=point,
            route="co2",
            ...
            refresh_pace_state=False,
            extra_fields=self._sealed_sweep_trace_extra(),
            note="first sealed CO2 sample completed; subsequent pressure points are setpoint-only",
        )

    def _co2_sealed_sweep_live_check_failures(self, point: CalibrationPoint) -> List[str]:
        failures: List[str] = []
        context = self._active_co2_sealed_sweep_context
        ...
        if context.get("route_sealed") is not True:
            failures.append("route_not_sealed")
        if context.get("key") != self._build_co2_sealed_sweep_key(point):
            failures.append("key_mismatch")
        if not self._co2_sealed_no_vent_guard_active:
            failures.append("no_vent_guard_inactive")
        if self.stop_event.is_set():
            failures.append("stop_event_set")
            ...
            try:
                ...
                if callable(get_vs):
                    ...
                    if self._pace_vent_status_is_diagnostic_only(vent_status):
                        ...
                        self._increment_sealed_sweep_counter("sealed_abnormal_vent_status_count")
                        failures.append(f"pace_vent_status_not_ready:{classification}")
            except Exception as exc:
                self._increment_sealed_sweep_counter("sealed_abnormal_vent_status_count")
                failures.append(f"pace_vent_status_unreadable:{exc}")
        return failures

    def _co2_sealed_guard_blocks_vent(self, *, action: str, reason: str = "") -> bool:
        if not self._co2_sealed_no_vent_guard_active:
            return False
        action_text = str(action or "").strip().upper()
        if "VENT ON" in action_text or "ATMOSPHERE" in action_text:
            self._increment_sealed_sweep_counter("sealed_vent1_count")
            self._increment_sealed_sweep_counter("vent_on_count")
            self._increment_sealed_sweep_counter("sealed_pressure_control_vent_on_count")
        elif "VENT OFF" in action_text:
            self._increment_sealed_sweep_counter("sealed_vent0_count")
            self._increment_sealed_sweep_counter("vent_off_count")
        self._increment_sealed_sweep_counter("no_vent_guard_blocked_count")
        self.log(
            f"CO2 sealed no-vent guard blocked {action}"
    ...
    def _query_pressure_controller_system_error(self, pace: Any) -> str:
        getter = getattr(pace, "get_system_error", None)
        if callable(getter):
            try:
        ...
        if callable(query):
            try:
                return str(query(":SYST:ERR?") or "").strip()
            except Exception as exc:
                return f"unavailable:{exc}"
        return "unavailable:no_method"

    @staticmethod
    def _pressure_controller_system_error_allows_continue(value: Any) -> bool:
        text = str(value or "").strip()
        if not text:
            return True
        if text.lower().startswith("unavailable:"):
            return True
    ...
    @staticmethod
    def _controlled_exit_fail_closed_decisions() -> set[str]:
        return {
            "FAIL_CLOSED_ATMOSPHERE_EXIT_COMMAND_FAILED",
            "FAIL_CLOSED_ATMOSPHERE_EXIT_NOT_VERIFIED",
    ...
    def _controlled_exit_failure_is_route_terminal(self) -> bool:
        ...
        return decision in self._controlled_exit_fail_closed_decisions()

    def _controlled_exit_atmosphere_snapshot(self, pace: Any) -> Dict[str, Any]:
        snapshot = dict(self._pace_state_snapshot(pace, refresh=True))
        output_mode: Any = ""
        mode_getter = getattr(pace, "get_output_mode", None)
        if callable(mode_getter):
            try:
                output_mode = mode_getter()
            except Exception as exc:
                output_mode = f"unavailable:{exc}"
        snapshot["pace_output_mode"] = output_mode
        snapshot["pace_system_error"] = self._query_pressure_controller_system_error(pace)
        return snapshot

    def _controlled_exit_atmosphere_note(
        self,
        *,
        result: str,
    ...
    def _controlled_operator_window_check_after_fixed_preseal_wait(
        self,
        point: CalibrationPoint,
        *,
        route: str,
        ...
        self._append_pressure_trace_row(
            ...
            note=(
                ...
                f"exit_method={exit_method}; "
                f"vent_status_after_vent0_classification={vent_status_classification}"
            ),
        )
        if not operator_final_decision and self._require_operator_window_clear_after_vent0():
        ...
        if operator_final_decision:
            self._controlled_exit_final_decision = operator_final_decision
            self.log(f"Controlled CO2 preseal operator window gate failed: {operator_final_decision}")
            return False
        return True
    ...
    def _read_com22_pressure_now(self) -> Optional[float]:
        gauge = self.devices.get("pressure_gauge")
        reader = getattr(gauge, "read_pressure", None) if gauge else None
        if not callable(reader):
            return None
    ...
    def _record_vent0_state_evidence(
        ...
        *,
        phase: str,
        vent0_command_ts: Optional[float],
        vent0_raw_response: Any = "",
        route_valves_still_open_during_wait: bool = True,
        fixed_wait_elapsed_s: Optional[float] = None,
        ...
        fields = {
            "vent0_command_ts": (
                ...
                if vent0_command_ts is not None
                else ""
            ),
            "vent0_raw_response": vent0_raw_response,
            "vent_status_after_vent0": vent_status,
            "vent_status_after_vent0_classification": classification,
            "outp_status_after_vent0": snapshot.get("pace_output_state"),
            "outp_mode_after_vent0": snapshot.get("pace_output_mode"),
            "isol_status_after_vent0": snapshot.get("pace_isolation_state"),
            "syst_err_after_vent0": snapshot.get("pace_system_error"),
            "pace_pressure_after_vent0_hpa": pace_pressure,
            "com22_pressure_after_vent0_hpa": com22_pressure,
            "route_valves_still_open_during_wait": route_valves_still_open_during_wait,
        ...
        self._append_pressure_trace_row(
            ...
            extra_fields=fields,
            note=(
                f"VENT0 evidence classification={classification}; "
                f"route_valves_still_open_during_wait={str(route_valves_still_open_during_wait).lower()}"
            ),
        )
        return fields
    ...
    def _verified_exit_atmosphere_for_controlled_co2_preseal(
        ...
        *,
        ...
        if (
            route_name != "co2"
            or not self._controlled_verified_exit_atmosphere_enabled(route_name)
            or self._co2_sealed_no_vent_guard_active
        ):
            return self._set_pressure_controller_vent(False, reason=reason)
        ...
        self._append_pressure_trace_row(
            ...
            route=route_name,
            point_phase=route_name,
            trace_stage="controlled_exit_atmosphere_begin",
            pressure_target_hpa=pressure_target_hpa,
        ...
        try:
            ...
            self._append_pressure_trace_row(
                ...
                route=route_name,
                point_phase=route_name,
                trace_stage="controlled_exit_atmosphere_fail",
                pressure_target_hpa=pressure_target_hpa,
                pace_output_state=snapshot.get("pace_output_state"),
                pace_isolation_state=snapshot.get("pace_isolation_state"),
                pace_vent_status=snapshot.get("pace_vent_status"),
            ...
            self._pressure_atmosphere_hold_enabled = False
            self._last_pressure_atmosphere_refresh_ts = 0.0
            self.log(f"Controlled CO2 preseal verified atmosphere exit command failed: {exc}")
            return False
        ...
        if failures:
            self.log(
                "Controlled CO2 preseal verified atmosphere exit failed: "
    ...
    def _set_pressure_to_target_in_active_co2_sealed_sweep(self, point: CalibrationPoint) -> bool:
        ...
        if not pace:
            return True
        if point.target_pressure_hpa is None:
            self.log("Missing target pressure, skipping CO2 sealed sweep setpoint-only transition.")
            return False
        ...
        self._append_pressure_trace_row(
            ...
            refresh_pace_state=False,
            extra_fields=self._sealed_sweep_trace_extra(),
            note="subsequent CO2 sealed pressure point; setpoint-only transition",
        )
        failures = self._co2_sealed_sweep_live_check_failures(point)
        if "key_mismatch" in failures:
            self._append_pressure_trace_row(
                point=point,
                route="co2",
                point_phase="co2",
        ...
        if failures:
            ...
            self._clear_active_co2_sealed_sweep_context(reason="sealed sweep live check failed", point=point)
            return False
        ...
        while time.time() - start < timeout_s:
            ...
            self._append_pressure_trace_row(
                ...
                route="co2",
                point_phase="co2",
                trace_stage="sealed_sweep_pressure_control_wait",
                pressure_target_hpa=target,
                pace_pressure_hpa=pressure_value,
                read_pressure_gauge=read_aux_now,
                read_dewpoint=read_aux_now,
            ...
            if inl == 1:
                self._append_pressure_trace_row(
                    ...
                    route="co2",
                    point_phase="co2",
                    trace_stage="sealed_sweep_pressure_in_limits",
                    pressure_target_hpa=target,
                    pace_pressure_hpa=pressure_value,
                    read_pressure_gauge=True,
                    read_dewpoint=True,
        ...
        self.log(f"CO2 sealed sweep pressure stabilize timeout at target {target} hPa")
        self._append_pressure_trace_row(
            point=point,
            route="co2",
            point_phase="co2",
            ...
            read_dewpoint=True,
            extra_fields=self._sealed_sweep_trace_extra(),
            note="pressure did not reach in-limits; no retry/reseal in sealed sweep",
        )
        self._clear_active_co2_sealed_sweep_context(reason="sealed sweep pressure timeout", point=point)
        return False
    ...
    def _pressure_recovery_safe_delta_hpa(self) -> float:
        return max(0.0, float(self._wf("workflow.pressure.output_recovery_safe_delta_hpa", 20.0) or 20.0))

    def _recovery_pressure_evidence_allows_output_path_action(
        self,
        point: CalibrationPoint,
        *,
        phase: str,
        ...
        com22_pressure = self._read_com22_pressure_now()
        reference_hpa = self._safe_stop_pressure_relief_reference_hpa()
        delta_limit_hpa = self._pressure_recovery_safe_delta_hpa()
        readings = [value for value in (pace_pressure, com22_pressure) if value is not None]
        max_abs_delta = max((abs(float(value) - float(reference_hpa)) for value in readings), default=None)
        evidence_available = bool(readings)
        allowed = bool(evidence_available and max_abs_delta is not None and max_abs_delta <= delta_limit_hpa)
        decision = "allow" if allowed else "block"
        self._append_pressure_trace_row(
            point=point,
            route=phase,
            point_phase=phase,
            trace_stage="control_output_on_recovery_pressure_evidence",
            pressure_target_hpa=pressure_target_hpa,
            pace_pressure_hpa=pace_pressure,
            pressure_gauge_hpa=com22_pressure,
            ...
            refresh_pace_state=True,
            note=(
                f"attempt={attempt_idx + 1} decision={decision} "
                f"reference_hpa={reference_hpa:.3f} delta_limit_hpa={delta_limit_hpa:.3f} "
                f"max_abs_delta_hpa={max_abs_delta if max_abs_delta is not None else 'NA'} "
                f"evidence_available={str(evidence_available).lower()}; {note}"
            ),
        )
        return allowed
    ...
    def _pace_vent_status_allows_control(self, pace: Any, vent_status: Any) -> bool:
        ...
        if status_value is None:
            return False
        return self._pace_vent_status_is_ready_for_control(status_value)
    ...
    def _enter_pressure_controller_atmosphere_with_legacy_hold(
        ...
        *,
        ...
        if self._no_outp_transition():
            route_baseline = self._co2_route_conditioning_baseline_reason(reason)
            baseline_fields: Dict[str, Any] = {}
            if route_baseline:
                self._last_co2_route_baseline_fields = {}
                baseline_fields = {
                    "route_baseline_output_state_before": "",
                    "route_baseline_outp0_sent": False,
                    "route_baseline_outp0_skipped_reason": "",
                    "route_baseline_isol_state_before": "",
                    "route_baseline_isol1_sent": False,
                    "route_baseline_isol1_skipped_reason": "",
                    "route_baseline_vent1_sent": False,
                    "route_baseline_atmosphere_hold_active": False,
                }
            stop_hold = getattr(pace, "stop_atmosphere_hold", None)
            if callable(stop_hold):
                stop_hold()
            if self._open_flow_output_off_for_reason(reason):
                ...
                send_outp0 = True
                if route_baseline:
                    output_state = self._read_pressure_controller_int_state(pace, "get_output_state")
                    baseline_fields["route_baseline_output_state_before"] = output_state
                    if output_state == 0:
                        send_outp0 = False
                        baseline_fields["route_baseline_outp0_skipped_reason"] = "already_off"
                    elif output_state is None and not self._route_baseline_send_on_unknown_state():
                        send_outp0 = False
                        baseline_fields["route_baseline_outp0_skipped_reason"] = "state_unknown_skip_by_policy"
                if callable(set_output) and send_outp0:
                    set_output(False)
                    if route_baseline:
                        baseline_fields["route_baseline_outp0_sent"] = True
                    self._append_pressure_trace_row(
                        point=None,
                        route="pressure",
                ...
                elif route_baseline and not callable(set_output):
                    baseline_fields["route_baseline_outp0_skipped_reason"] = "set_output_unavailable"
            set_isolation_open = getattr(pace, "set_isolation_open", None)
            send_isol1 = True
            if route_baseline:
                isolation_state = self._read_pressure_controller_int_state(pace, "get_isolation_state")
                baseline_fields["route_baseline_isol_state_before"] = isolation_state
                if isolation_state == 1:
                    send_isol1 = False
                    baseline_fields["route_baseline_isol1_skipped_reason"] = "already_open"
                elif isolation_state is None and not self._route_baseline_send_on_unknown_state():
                    send_isol1 = False
                    baseline_fields["route_baseline_isol1_skipped_reason"] = "state_unknown_skip_by_policy"
            if callable(set_isolation_open) and send_isol1:
                set_isolation_open(True)
                if route_baseline:
                    baseline_fields["route_baseline_isol1_sent"] = True
            elif route_baseline and not callable(set_isolation_open):
                baseline_fields["route_baseline_isol1_skipped_reason"] = "set_isolation_open_unavailable"
            pace.vent(True)
            if route_baseline:
                baseline_fields["route_baseline_vent1_sent"] = True
            start_hold = getattr(pace, "start_atmosphere_hold", None)
            if callable(start_hold):
                start_hold(interval_s=self._vent_hold_interval_s())
            if route_baseline:
                baseline_fields["route_baseline_atmosphere_hold_active"] = self._pressure_controller_atmosphere_hold_active(
                    pace
                )
                self._last_co2_route_baseline_fields = dict(baseline_fields)
            self._pace_vent_after_valve_supported = False
    ...
    def _set_pressure_controller_vent(self, vent_on: bool, reason: str = "") -> bool:
        ...
        if not pace:
            return True
        if self._co2_sealed_guard_blocks_vent(
            action=f"VENT {'ON' if vent_on else 'OFF'}",
            reason=reason,
        ):
            return False
    ...
    def _refresh_pressure_controller_atmosphere_hold(self, *, force: bool = False, reason: str = "") -> None:
        ...
        if not pace:
            return
        if self._co2_sealed_guard_blocks_vent(action="atmosphere refresh", reason=reason):
            return
    ...
    def _enable_controlled_outp_after_seal(
        ...
        *,
        ...
        if pace:
            current_output_state: Any = None
            output_state_error = ""
            get_output_state = getattr(pace, "get_output_state", None)
            if not callable(get_output_state):
                output_state_error = "get_output_state_unavailable"
            else:
                try:
                    current_output_state = int(get_output_state())
                except Exception as exc:
                    output_state_error = str(exc) or "get_output_state_failed"
            if current_output_state == 1:
                ...
                self._append_pressure_trace_row(
                    ...
                    route="co2",
                    point_phase="co2",
                    trace_stage="sealed_control_output_already_on",
                    pressure_target_hpa=pressure_target_hpa,
                    pace_output_state=current_output_state,
                    refresh_pace_state=False,
                    extra_fields=self._sealed_sweep_trace_extra(),
                    note="output_state already 1 after route seal; skip duplicate OUTP1",
                )
                return True
            if current_output_state is None:
                self._append_pressure_trace_row(
                    point=point,
                    route="co2",
                    point_phase="co2",
                    ...
                    refresh_pace_state=False,
                    extra_fields=self._sealed_sweep_trace_extra(),
                    note=f"output_state unknown before OUTP1; block blind enable ({output_state_error})",
                )
                return False
    ...
    def _attempt_pressure_controller_output_on_recovery(
        ...
        *,
        ...
        for attempt_idx in range(retries):
            ...
            if not self._recovery_pressure_evidence_allows_output_path_action(
                point,
                phase=phase,
                pressure_target_hpa=pressure_target_hpa,
            ...
            if not self._no_outp_transition():
                ...
                if callable(set_output):
                    try:
                        if self._co2_sealed_no_vent_guard_active:
                            self._increment_sealed_sweep_counter("sealed_outp0_count")
                            self._increment_sealed_sweep_counter("output_disable_count")
                        set_output(False)
                    except Exception as exc:
    ...
    def _cached_actual_open_valves(self) -> List[int]:
        open_valves: List[int] = []
        for valve in self._managed_valves():
            relay_name, channel = self._resolve_valve_target(valve)
            state = self._relay_state_cache.get((relay_name, int(channel)))
            if state is True:
                open_valves.append(int(valve))
        return sorted(open_valves)
    ...
    def _run_co2_point(
        ...
        self._apply_co2_pre_route_idle_baseline(reason="before CO2 chamber wait")
        self._emit_stage_event(
        ...
        route_open_ok = self._open_co2_route_for_conditioning(point, point_tag=route_point_tag)
        if route_open_ok is False:
            decision = str(
                self._controlled_exit_final_decision
                or self._co2_route_terminal_failure_decision
                or "FAIL_CLOSED_ROUTE_OPEN_PACE_ATMOSPHERE_NOT_CLEAN"
            )
            self.log(f"CO2 row {point.index} skipped: route-open atmosphere gate failed ({decision})")
            self._cleanup_co2_route(reason=decision)
            return
        ...
        for pressure_idx, pressure_point in enumerate(active_pressure_refs):
            ...
            if pressure_idx == 0 or self._active_co2_sealed_sweep_context is None:
            ...
            if not pressure_ok:
                ...
                self._clear_active_co2_sealed_sweep_context(reason="CO2 sealed pressure point failed", point=sample_point)
                self._cleanup_co2_route(reason="after CO2 sealed pressure point failure")
                return
            ...
            if not self._wait_after_pressure_stable_before_sampling(sample_point):
                ...
                self._clear_active_co2_sealed_sweep_context(reason="CO2 sealed post-pressure hold interrupted", point=sample_point)
                self._cleanup_co2_route(reason="after CO2 sealed post-pressure hold interrupted")
                return
            ...
            if pressure_idx == 0 and len(active_pressure_refs) > 1:
                self._begin_active_co2_sealed_sweep_context(sample_point)
    ...
    def _set_co2_route_baseline(self, *, reason: str = "") -> bool:
        route_baseline = self._co2_route_conditioning_baseline_reason(reason)
        if route_baseline:
            self._last_co2_route_baseline_fields = {}
        vent_ok = self._set_pressure_controller_vent(True, reason=reason)
        self._apply_route_baseline_valves()
        if route_baseline:
            fields = dict(self._last_co2_route_baseline_fields)
            fields["route_baseline_atmosphere_hold_active"] = self._pressure_controller_atmosphere_hold_active(
            ...
            self._append_pressure_trace_row(
                ...
                route="co2",
                point_phase="co2",
                trace_stage="route_conditioning_baseline",
                refresh_pace_state=False,
                extra_fields=fields,
                note="before CO2 route conditioning",
            )
    ...
    def _apply_co2_pre_route_idle_baseline(self, *, reason: str = "") -> None:
        self._apply_route_baseline_valves()
        fields = {
            "co2_pre_route_idle_isolation_skipped": True,
            "skipped_reason": "co2_open_flow_requires_continuous_atmosphere_before_route_open",
            "pace_vent0_before_route_open_suppressed": True,
        }
        self._append_pressure_trace_row(
            point=None,
            route="co2",
            point_phase="co2",
            trace_stage="co2_pre_route_idle_isolation_skipped",
            refresh_pace_state=False,
            extra_fields=fields,
            note=f"{reason}: PACE VENT0 suppressed before CO2 open-flow route open",
        )
        extra = f" ({reason})" if str(reason or "").strip() else ""
        self.log(
            "CO2 pre-route idle baseline applied"
            f"{extra}: route valves closed; pressure controller atmosphere state preserved"
        )
        ...
        while True:
            ...
            if bool(gate_eval.get("gate_pass")):
                ...
                self._append_pressure_trace_row(
                    ...
                    flush_gate_reason="",
                    extra_fields={
                        "dewpoint_gate_pass_ts": datetime.fromtimestamp(
    ...
    def _wait_co2_preseal_primary_sensor_gate(self, point: CalibrationPoint) -> bool:
        ...
        self._append_pressure_trace_row(
            ...
            extra_fields={
                ...
                "analyzer_gate_end_ts": datetime.fromtimestamp(
    ...
    def _co2_route_open_clean_atmosphere_gate(self, point: CalibrationPoint, *, point_tag: str = "") -> bool:
        pace = self.devices.get("pace")
        if pace is None:
            fields = {
                "route_open_allowed": True,
                "route_open_block_reason": "pace_unavailable_gate_skipped",
                "actual_open_valves_before_route_open": ",".join(str(v) for v in self._cached_actual_open_valves()),
            }
            self._append_pressure_trace_row(
                point=point,
                route="co2",
                point_phase="co2",
                point_tag=point_tag,
                trace_stage="route_open_clean_atmosphere_gate",
                pressure_target_hpa=point.target_pressure_hpa,
                refresh_pace_state=False,
                extra_fields=fields,
                note="route-open PACE clean atmosphere gate skipped: pace unavailable",
            )
            return True
        ...
        reference = self._atmosphere_reference_hpa
        ...
        self._append_pressure_trace_row(
            ...
            refresh_pace_state=False,
            extra_fields=fields,
            note="pass" if allowed else block_reason,
        )
        if allowed:
            return True
        self._mark_co2_route_terminal_failure(
            final_decision="FAIL_CLOSED_ROUTE_OPEN_PACE_ATMOSPHERE_NOT_CLEAN",
            reason=block_reason or "route-open PACE clean atmosphere gate failed",
            point=point,
            phase="co2",
        )
        return False
    ...
    def _preseal_dewpoint_freshness_max_age_s(self) -> float:
        return max(
            0.0,
            float(self._wf("workflow.stability.dewpoint_preseal_freshness_max_age_s", 60.0) or 60.0),
        )
    ...
    def _pressurize_and_hold(self, point: CalibrationPoint, route: str = "co2") -> bool:
        ...
        try:
                ...
                if (
                    ...
                    if remaining_s > 0:
                        self._append_pressure_trace_row(
                            ...
                            route=phase,
                            point_phase=phase,
                            trace_stage="co2_vent0_open_route_wait_before_seal",
                            pressure_target_hpa=point.target_pressure_hpa,
                            refresh_pace_state=False,
                            note=(
                                f"route valves remain open after VENT0; "
                                f"elapsed_s={elapsed_s:.3f} remaining_s={remaining_s:.3f}"
                            ),
                        )
            ...
            vent0_command_ts = time.time()
            self._last_vent0_command_ts = vent0_command_ts
            if self._controlled_verified_exit_atmosphere_enabled(route_name):
                if not self._verified_exit_atmosphere_for_controlled_co2_preseal(
                    point,
                    route=route_name,
            ...
            if (
                ...
                window_snapshot = self._controlled_exit_atmosphere_snapshot(pace)
                exit_method = "pace.exit_atmosphere_mode" if callable(getattr(pace, "exit_atmosphere_mode", None)) else "manual_fallback"
                if not self._controlled_operator_window_check_after_fixed_preseal_wait(
                    point,
                    route=phase,
                    snapshot=window_snapshot,
                    exit_method=exit_method,
                    reason=vent_off_reason,
                    wait_status=window_snapshot.get("pace_vent_status"),
                    wait_error="",
                ):
                    self._cleanup_co2_route(reason=str(self._controlled_exit_final_decision or "operator window gate failure"))
            ...
            if self._no_outp_transition() and route_name == "co2":
                ...
                if pace:
                    ...
                    while time.time() - rise_start < rise_timeout_s:
                        ...
                        self._stop_pressure_transition_fast_signal_context(reason="preseal pressure-rise gate failure")
                        return False
    ...
    def _sample_and_log(self, point: CalibrationPoint, phase: str = "", point_tag: str = "") -> None:
        ...
        if phase_text == "co2" and self._co2_sealed_no_vent_guard_active:
            self._increment_sealed_sweep_counter("sealed_sample_count")
            self._append_pressure_trace_row(
                point=point,
                route=phase_text,
                point_phase=phase_text,
                point_tag=point_tag,
                trace_stage="sealed_sweep_sample_counted",
                pressure_target_hpa=point.target_pressure_hpa,
                pace_pressure_hpa=self._as_float(last_sample.get("pressure_hpa")),
                pressure_gauge_hpa=self._as_float(last_sample.get("pressure_gauge_hpa")),
                pace_output_state=self._as_int(last_sample.get("pace_output_state")),
                ...
                refresh_pace_state=False,
                extra_fields=self._sealed_sweep_trace_extra(),
                note="CO2 sealed pressure sample completed with no-vent guard active",
            )
