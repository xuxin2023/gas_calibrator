...
| 正式流程编排 (`full_flow_orchestration`) | 10 | 用于确认 V1.5 顺序和证据合同；默认不打开 COM，不直接控制设备。 |
| 压力 no-write 跑器 (`formal_pressure_no_write_runner`) | 1 | 只用于压力通道验证/校准证据；压力 P 独立处理，不能混入 CO2/H2O 主拟合。 |
| 离线证据和评审 (`formal_review_evidence`) | 206 | 用于候选系数、报告、归档、数据库和复核；不能替代真实采样。 |
...
| 旧 V1 参考入口 (`legacy_v1_reference`) | 3 | 只保留历史算法和审计参考；不要用于启动 V1.5 正式校准。 |
| 证据数据库 (`evidence_database`) | 12 | 保存索引、hash、追溯关系和查询能力；原始文件仍是证据包的一部分。 |
| 高级质控 (`advanced_qc`) | 10 | 解释稳定性、湿度、压力、工厂信号和根因；阈值调整必须留痕。 |
| 只读/评审界面 (`ui_review`) | 11 | 显示流程状态和证据，不应暗示可直接真实控制设备。 |
| 测试门禁 (`test_gate`) | 269 | 保护 V1.5 合同、证据口径和防误用规则。 |
...
| `legacy_v1_reference_only` | 3 | 旧 V1 入口只作为历史参考，不用于 V1.5 正式流程。 |
| `archive_housekeeping_only` | 1 | 只做整理或归档辅助，不是校准数据生成入口。 |
| `classification_required` | 1 | 若出现，说明新增文件还没有明确身份，不能投入使用。 |
