...
- 当前识别到 V1.5 相关入口/模块/测试共 `216` 个。
- 正式主路以 `11` 个 canonical 阶段为准。
...
| 2 | `01_formal_initialization` | `src/gas_calibrator/tools/run_v1_5_formal_initialization_runner.py` | Owns the formal initialization contract: device-ID binding, GETCO1-9 epoch-0 snapshot, S5/S6/S7/S8/S9 gates, startup acquisition settings, readiness, and database evidence indexing. |
...
| 离线证据和评审 (`formal_review_evidence`) | 64 | 用于候选系数、报告、归档、数据库和复核；不能替代真实采样。 |
| 受控写入 (`controlled_write`) | 11 | 只在评审通过后手动授权执行，必须有旧值、候选值、读回和复验。 |
| 工程诊断 (`diagnostic_only`) | 9 | 可以保留证据，但默认不进入 formal acceptance，也不进入 CO2/H2O 正式拟合。 |
...
| 测试门禁 (`test_gate`) | 95 | 保护 V1.5 合同、证据口径和防误用规则。 |

## 不要从这里启动正式流程

| 防误用规则 | 数量 | 含义 |
|---|---:|---|
| `authorized_write_only` | 11 | 只能在候选系数评审和授权后写入，不能被报告或归档自动调用。 |
| `diagnostic_not_acceptance` | 9 | 只能用于工程诊断，默认不能作为正式验收或正式拟合数据。 |
