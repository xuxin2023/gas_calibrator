...
from gas_calibrator.tools.export_v1_5_authoritative_resume_offline_candidate_gate import (
    main,
)
...
def _bundle(tmp_path: Path, *, target_index: int = 4) -> dict[str, Path]:
    config = _write(tmp_path / "config.json", {})
    root = tmp_path / "flow"
    plan_model = build_full_flow_plan(config_path=config, output_dir=root, run_id="resume-103")
    plan = write_full_flow_plan(plan_model, root)["json"]
    completed = [step.step_id for step in plan_model.steps[:target_index]]
    ...
    authorization = _write(
        root / "authorization.json",
        {
            ...
            "authorization_id": "resume-auth-103",
            "operator": "operator-a",
            "reviewer": "reviewer-b",
            "approver": "approver-c",
    ...
    preflight_model = build_v1_5_authoritative_resume_execution_preflight(
        authorization_validation_json=validation,
        now=NOW + timedelta(seconds=10),
    )
    preflight = write_v1_5_authoritative_resume_execution_preflight(
        preflight_model, root / "preflight"
    )["json"]
    return {"preflight": preflight}


def test_gate_accepts_fresh_canonical_offline_step_without_execution(tmp_path: Path) -> None:
    bundle = _bundle(tmp_path)
    model = build_v1_5_authoritative_resume_offline_candidate_gate(
        execution_preflight_json=bundle["preflight"],
        now=NOW + timedelta(seconds=20),
    )
    assert model["overall_status"] == READY_STATUS
    assert model["offline_resume_candidate_ready"] is True
    assert model["next_step_execution_mode"].startswith("offline")
...
def test_gate_rejects_physical_read_only_com_next_step(tmp_path: Path) -> None:
    bundle = _bundle(tmp_path, target_index=17)
    model = build_v1_5_authoritative_resume_offline_candidate_gate(
        execution_preflight_json=bundle["preflight"],
        now=NOW + timedelta(seconds=20),
    )
    assert model["overall_status"] == REVIEW_STATUS
    assert model["offline_resume_candidate_ready"] is False
    assert "canonical_next_step_not_offline_mode" in model["review_reasons"]
    assert "canonical_next_step_side_effect:opens_com_ports" in model["review_reasons"]
    assert model["physical_or_write_step_must_use_dedicated_executor"] is True


def test_gate_rejects_stale_preflight_even_when_authorization_remains_valid(
    tmp_path: Path,
) -> None:
    bundle = _bundle(tmp_path)
    model = build_v1_5_authoritative_resume_offline_candidate_gate(
    ...
    assert model["overall_status"] == REVIEW_STATUS
    assert "execution_preflight_not_fresh" in model["review_reasons"]


def test_gate_rejects_tampered_preflight_command(tmp_path: Path) -> None:
    bundle = _bundle(tmp_path)
    payload = json.loads(bundle["preflight"].read_text(encoding="utf-8"))
    payload["next_step_command_recorded_only"] = ["python", "-m", "tampered"]
    _write(bundle["preflight"], payload)
    model = build_v1_5_authoritative_resume_offline_candidate_gate(
        execution_preflight_json=bundle["preflight"],
        now=NOW + timedelta(seconds=20),
    )
    assert model["overall_status"] == REVIEW_STATUS
    assert any(
        reason.startswith("execution_preflight_recompute_mismatch")
        for reason in model["review_reasons"]
    )
    assert "canonical_next_step_command_mismatch" in model["review_reasons"]


def test_gate_rejects_empty_canonical_command() -> None:
    reasons = _offline_step_reasons(
        {
            "step_id": "offline-empty",
            "execution_mode": "offline_sidecar",
            "tool_module": "gas_calibrator.tools.export_example",
            "command": [],
    ...
    assert "canonical_next_step_command_missing" in reasons


def test_gate_cli_and_entrypoint_remain_offline(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bundle = _bundle(tmp_path)
    monkeypatch.setattr(
        "gas_calibrator.validation.v1_5_authoritative_resume_offline_candidate_gate._now",
        lambda: NOW + timedelta(seconds=20),
    )
    ...
    assert (output / "v1_5_authoritative_resume_offline_candidate_gate.json").is_file()
    entry = classify_v1_5_entrypoint(
        ROOT
        / "src/gas_calibrator/tools/export_v1_5_authoritative_resume_offline_candidate_gate.py",
        root=ROOT,
    )
...
def test_gate_cli_rejects_runtime_unlocks_before_output(tmp_path: Path, flag: str) -> None:
    bundle = _bundle(tmp_path)
