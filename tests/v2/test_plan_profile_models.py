...
def test_mode_model_types_and_functions_have_one_package_owner() -> None:
    assert ModeProfile.__module__ == "gas_calibrator.validation.simulation.domain"
    assert RunMode.__module__ == "gas_calibrator.validation.simulation.domain"
    assert normalize_run_mode.__module__ == "gas_calibrator.validation.simulation.domain"
    assert run_mode_label.__module__ == "gas_calibrator.validation.simulation.domain"


def test_plan_profile_types_have_one_shared_validation_owner() -> None:
    plan_types = (
        AnalyzerSetupSpec,
        CalibrationPlanProfile,
        GasPointSpec,
    ...
    assert all(
        item.__module__ == "gas_calibrator.validation.simulation.plan_models"
    ...
    assert not (old_domain / "plan_models.py").exists()
    assert all(not hasattr(domain, item.__name__) for item in plan_types)


def test_plan_compiler_does_not_redefine_or_reexport_shared_plan_models() -> None:
    from gas_calibrator.v2.core import plan_compiler
    ...
    assert all(not hasattr(plan_compiler, name) for name in shared_names)
