...
def test_artifact_service_exports_ratio_poly_report(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "gas_calibrator.v2.core.services.artifact_service.export_ratio_poly_report",
        fake_export,
    )

    service = ArtifactService(context, run_state, host=host)
    result = service.export_coefficient_report()

    assert result == {"status": "ok", "path": str(output_path)}
...
def test_coefficient_service_module_does_not_reappear() -> None:
    source_root = Path(__file__).resolve().parents[2] / "src"

    assert not (
        source_root
        / "gas_calibrator/v2/core/services/coefficient_service.py"
    ).exists()
    assert "CoefficientService" not in services.__all__
    assert not hasattr(services, "CoefficientService")
