...
def _make_service(host, ambient_hpa=None):
    ...
    service._ambient_reference_payload = lambda: {
        "ambient_reference_pressure_hpa": ambient_hpa,
        "ambient_reference_source": "test",
        "ambient_reference_timestamp": "2026-05-07T00:00:00+00:00",
    }
    service._positive_preseal_enabled = lambda *a, **kw: True
    ...
    service._positive_preseal_pressurization = lambda *a, **kw: PressureWaitResult(
        ok=True,
        diagnostics={
            ...
            "vent_close_command_sent_at": "2026-05-07T00:00:02+00:00",
        },
    )
...
class TestSubAtmosphericSkipsPositivePreseal:
    def test_800hPa_with_ambient_1015_skips_preseal(self):
        ...
        host, traces = _make_host({
            ...
            "workflow.pressure.pressurize_wait_after_vent_off_s": 0.01,
        })
        service = _make_service(host, ambient_hpa=1015.0)
        ...
        assert ev["positive_preseal_skipped"] is True
        assert ev["positive_preseal_skip_reason"] == "target_below_atmosphere_direct_pressure_control"
        assert ev["pressure_read_between_vent_off_and_route_close"] is False

    def test_800hPa_vent_off_to_route_close_under_limit(self):
        point = _point(pressure=800.0)
        host, traces = _make_host({
            "workflow.pressure.positive_preseal_pressurization_enabled": True,
            "workflow.pressure.co2_preseal_sub_atmospheric_margin_hpa": 50.0,
        ...
        assert ev["vent_off_to_route_close_s"] <= 1.5

    def test_1100hPa_still_uses_preseal(self):
        ...
        assert ev["positive_preseal_skipped"] is False
        assert ev["positive_preseal_skip_reason"] == ""
        assert ev["positive_preseal_peak_hpa"] == 1110.0

    def test_800hPa_no_ambient_skips_when_below_1000(self):
        point = _point(pressure=800.0)
        host, traces = _make_host({
            "workflow.pressure.positive_preseal_pressurization_enabled": True,
            "workflow.pressure.co2_preseal_sub_atmospheric_margin_hpa": 50.0,
        ...
        service = _make_service(host, ambient_hpa=None)
    ...
    def test_1100hPa_no_ambient_still_preseals(self):
        point = _point(pressure=1100.0)
        host, traces = _make_host({
            "workflow.pressure.positive_preseal_pressurization_enabled": True,
            "workflow.pressure.co2_preseal_sub_atmospheric_margin_hpa": 50.0,
...
class TestH2oUnaffected:
    def test_h2o_800hPa_does_not_skip(self):
