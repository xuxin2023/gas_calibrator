...
from gas_calibrator.tools.run_v1_5_dewpoint_only_hold_after_gate import (
    DEWPOINT_ONLY_HOLD_NO_REBOUND,
    DEWPOINT_ONLY_HOLD_REBOUND_OBSERVED,
    DewpointOnlyHoldDiagnosticRunner,
...
class _ForbiddenAnalyzer:
    def read_latest_data(self, *_args: Any, **_kwargs: Any) -> str:
        raise AssertionError("dewpoint-only hold must not read analyzer data")
...
class _ForbiddenPace:
    ...
    def vent(self, *_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("dewpoint-only hold must not write VENT0/VENT1 directly")

    def set_isolation_open(self, *_args: Any, **_kwargs: Any) -> None:
        raise AssertionError("dewpoint-only hold must not write ISOL")
...
def _runner(tmp_path: Path, dewpoints: Iterable[float]) -> tuple[DewpointOnlyHoldDiagnosticRunner, RunLogger, CalibrationPoint]:
    cfg = _cfg(tmp_path)
    logger = RunLogger(tmp_path, run_id="unit", cfg=cfg)
    point = _point()
    runner = DewpointOnlyHoldDiagnosticRunner(
        cfg,
        {
...
def _summary(logger: RunLogger) -> dict:
    path = logger.run_dir / "dewpoint_only_hold_summary.json"
    return json.loads(path.read_text(encoding="utf-8"))


def test_dewpoint_only_hold_does_not_enter_analyzer_gate(tmp_path: Path) -> None:
    ...
    assert _summary(logger)["final_decision"] == DEWPOINT_ONLY_HOLD_NO_REBOUND


def test_dewpoint_only_hold_does_not_send_vent0_or_outp(tmp_path: Path) -> None:
...
def test_dewpoint_only_hold_keeps_vent1_and_route_open(tmp_path: Path) -> None:
    runner, logger, point = _runner(tmp_path, [-33.6])
...
def test_dewpoint_only_hold_records_rebound_vs_tail_reference(tmp_path: Path) -> None:
...
def test_dewpoint_only_hold_rebound_observed_decision(tmp_path: Path) -> None:
    ...
    assert _summary(logger)["final_decision"] == DEWPOINT_ONLY_HOLD_REBOUND_OBSERVED


def test_dewpoint_only_hold_no_rebound_decision(tmp_path: Path) -> None:
...
def test_dewpoint_only_hold_is_no_write(tmp_path: Path) -> None:
    runner, logger, point = _runner(tmp_path, [-33.6])
...
def test_no_change_to_normal_runner_path() -> None:
    assert (
        DewpointOnlyHoldDiagnosticRunner._wait_co2_preseal_primary_sensor_gate
        is not CalibrationRunner._wait_co2_preseal_primary_sensor_gate
    )
